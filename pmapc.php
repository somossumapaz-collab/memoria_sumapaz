        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            if (btnProcessTranscript) {
                btnProcessTranscript.addEventListener('click', async () => {
                    ...
                    const isJsonFile = (selectedFile && selectedFile.name.toLowerCase().endsWith('.json')) || loadedTranscriptText.trim().startsWith('{');

                    if (isJsonFile) {
                        try {
                            const parsed = JSON.parse(loadedTranscriptText);
                            ...
                            // Match/determine producer
                            let targetPid = parsed.productor_id || pmapcData.productor_id || selectedProducerId;
                            
                            if (!targetPid && typeof allProducers !== 'undefined' && allProducers && allProducers.length > 0) {
                                ...
                                const match = allProducers.find(p => {
                                    const pNombre = (p.nombre_completo || '').toLowerCase().trim();
                                    const pOrg = (p.nombre_organizacion || '').toLowerCase().trim();
                                    return (pName && (pNombre.includes(pName) || pName.includes(pNombre))) || (orgName && pOrg && (pOrg.includes(orgName) || orgName.includes(pOrg)));
                                });
                                if (match) {
                                    targetPid = match.id;
                                }
                            ...
                            if (targetPid) {
                                selectedProducerId = targetPid;
                                const pInput = document.getElementById('productor_id');
                                if (pInput) pInput.value = targetPid;
                                
                                const prod = allProducers.find(p => p.id == targetPid);
                                if (prod) {
                                    const searchInput = document.getElementById('select-producer-input');
                            ...
                            if (selectedProducerId) {
                                ...
                                alert("¡Formulario autocompletado! Selecciona un Productor en el buscador para guardar directamente en la base de datos.");
                            }
                        } catch (err) {

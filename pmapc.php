...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            let lastLoadedPreguntasRespuestas = null;
        ...
        function parseJsonSafely(str) {
            if (!str || typeof str !== 'string') throw new Error("El archivo seleccionado está vacío.");
            ...
            try {
                ...
                console.warn("Direct JSON.parse failed, attempting automatic structural fixes...", e1.message);
                
                let fixed = clean;

                // A. Fix missing commas between array objects: } \n { -> }, {
                fixed = fixed.replace(/}\s*[\r\n]+\s*{/g, '},{');
                fixed = fixed.replace(/}\s+{/g, '},{');

                // B. Fix missing commas between JSON properties: "value" \n "key": -> "value", \n "key":
                fixed = fixed.replace(/("[\s\S]*?")\s*([\r\n]+\s*"[a-zA-Z0-9_]+"\s*:)/g, '$1,$2');

                // C. Fix trailing commas before } or ]
                ...
                // D. Fix unescaped newlines/tabs inside double-quoted strings
                ...
                // E. Fix smart/curly quotes
                fixed = fixed.replace(/[“”]/g, '"').replace(/[‘’]/g, "'");
                fixed = fixed.replace(/,\s*([}\]])/g, '$1').replace(/}\s*[\r\n]+\s*{/g, '},{');

                try {
                    return JSON.parse(fixed);
                } catch (e4) {}

                // F. Fallback error message
                throw new Error("El archivo JSON contiene un error de sintaxis (" + e1.message + "). Revisa comillas o comas faltantes.");
            }
        }
            ...
            if (btnProcessTranscript) {
                btnProcessTranscript.addEventListener('click', async () => {
                    ...
                    const selectedFile = transcriptFileInput.files ? transcriptFileInput.files[0] : null;
                    const isJsonFile = (selectedFile && selectedFile.name.toLowerCase().endsWith('.json')) || loadedTranscriptText.trim().startsWith('{') || loadedTranscriptText.trim().startsWith('```');

                    if (isJsonFile) {
                        try {
                            const parsed = parseJsonSafely(loadedTranscriptText);
                            const pmapcData = parsed.data || parsed;

                            if (pmapcData.preguntas_respuestas) {
                                lastLoadedPreguntasRespuestas = pmapcData.preguntas_respuestas;
                            }

                            // Match/determine valid producer from database
                            const findProducer = (id) => (id && typeof allProducers !== 'undefined' && allProducers) ? allProducers.find(p => p.id == id) : null;

                            let validProd = findProducer(parsed.productor_id || pmapcData.productor_id);

                            if (!validProd && typeof allProducers !== 'undefined' && allProducers && allProducers.length > 0) {
                            ...
                            // If not matched by JSON content, check if user already selected a valid producer in the UI
                            if (!validProd && selectedProducerId) {
                                validProd = findProducer(selectedProducerId);
                            }

                            if (validProd) {
                                selectedProducerId = validProd.id;
                                const pInput = document.getElementById('productor_id');
                                if (pInput) pInput.value = validProd.id;
                                
                                const searchInput = document.getElementById('select-producer-input');
                                if (searchInput) searchInput.value = validProd.nombre_completo;
                                showProducerDetails(validProd);
                            } else {
                                selectedProducerId = null;
                                const pInput = document.getElementById('productor_id');
                            ...
                            // Populate form
                            populateForm(pmapcData);

                            // Auto save immediately to DB (Overwriting existing record if present)
                            if (selectedProducerId) {
                                await submitForm();
                            } else {
                                alert("¡Formulario autocompletado con éxito! Por favor selecciona un Productor en el buscador superior para guardar directamente en la base de datos.");
                            }
                        } catch (err) {
                    ...
                    if (!selectedProducerId) {
                        alert("Por favor selecciona un productor antes de procesar el archivo con IA.");
                        return;
                    }
        ...
        function populateForm(data) {
            ...
            // 1. Process preguntas_respuestas array if present (e.g. PMAPC_Plantilla_Exhaustiva.json format)
            if (data.preguntas_respuestas && Array.isArray(data.preguntas_respuestas)) {
                data.preguntas_respuestas.forEach(item => {
                    if (!item.campo || item.respuesta === undefined || item.respuesta === null) return;
                    const val = item.respuesta;
                    setVal(item.campo, val);
                    setInputByName(item.campo, val);
                    if (item.campo === 'pdf_comentarios' || item.campo === 'comentarios') {
                        setVal('pdf_comentarios', val);
                    }
                });
            }

            // Top-level / Metadata fields
            if (data.nombre_organizacion) setVal('f01_nombre_organizacion', data.nombre_organizacion);
            if (data.productora) {
                const lblNombre = document.getElementById('lbl-nombre');
                if (lblNombre && (!lblNombre.textContent || lblNombre.textContent === '-')) {
                    lblNombre.textContent = data.productora;
                }
            }
            if (data.vereda) setVal('edit-vereda', data.vereda);
        ...
        async function submitForm() {
            ...
            const productorId = selectedProducerId;
            ...
            let prod = (typeof allProducers !== 'undefined' && allProducers) ? allProducers.find(p => p.id == selectedProducerId) : null;
            if (!prod) {
                const nameEl = document.getElementById('lbl-nombre');
                const docEl = document.getElementById('lbl-documento');
                const docText = docEl ? docEl.textContent : '';
                
                prod = {
                    id: selectedProducerId,
                    nombre_completo: (nameEl && nameEl.textContent && nameEl.textContent !== '-') ? nameEl.textContent : 'Productor',
                    tipo_documento: docText.split(' ')[0] || 'CC',
                    numero_documento: docText.split(' ')[1] || ''
                };
            }
            ...
            if (typeof lastLoadedPreguntasRespuestas !== 'undefined' && lastLoadedPreguntasRespuestas) {
                payload.data.preguntas_respuestas = lastLoadedPreguntasRespuestas;
            }

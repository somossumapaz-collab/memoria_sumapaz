...
$json_path = 'C:\Users\sotoc\Downloads\JSON Alba Nery Mican.json';
...
if (json_last_error() !== JSON_ERROR_NONE) {
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        die("Fatal JSON syntax error in Alba Nery JSON: " . json_last_error_msg() . "\n");
    } else {
        echo "Fixed JSON syntax successfully!\n";
    }
}

echo "=== INSPECTING ALBA NERY MICAN POVEDA JSON ===\n";
...
// Find Alba Nery Mican Poveda in productores_sumapaz
$stmt = $pdo->query("SELECT id, nombre_completo, vereda, numero_documento FROM productores_sumapaz WHERE nombre_completo LIKE '%Alba%' OR nombre_completo LIKE '%Nery%'");
...
foreach ($producers as $p) {
    if (strpos(strtolower($p['nombre_completo']), 'alba') !== false && strpos(strtolower($p['nombre_completo']), 'nery') !== false) {
        $alba_id = $p['id'];
        break;
    }
}

if (!$alba_id && !empty($producers)) {
    $alba_id = $producers[0]['id'];
}

if (!$alba_id) {
    die("Alba Nery Mican Poveda not found in database!\n");
}

echo "\nTargeting Producer ID: {$alba_id}\n";

// Insert / Update pmapc_registros
$stmtCheck = $pdo->prepare("SELECT id FROM pmapc_registros WHERE productor_id = ?");
$stmtCheck->execute([$alba_id]);
...
if ($existingId) {
    ...
    echo "Successfully UPDATED existing PMAPC record ID {$existingId} for producer {$alba_id}.\n";
} else {
    $stmtIn = $pdo->prepare("INSERT INTO pmapc_registros (productor_id, data) VALUES (?, ?)");
    $stmtIn->execute([$alba_id, $jsonData]);
    echo "Successfully INSERTED new PMAPC record for producer {$alba_id}.\n";
}

// Verify payload length
$stmtVer = $pdo->prepare("SELECT CHAR_LENGTH(data) FROM pmapc_registros WHERE productor_id = ?");
$stmtVer->execute([$alba_id]);

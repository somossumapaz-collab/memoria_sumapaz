...
path = r'C:\Users\sotoc\Downloads\JSON Andrea Paola Ramirez Parra.json'
if not os.path.exists(path):
    files = [f for f in os.listdir(r'C:\Users\sotoc\Downloads') if 'Andrea' in f]
    if files:
...
try:
    ...
    print("SUCCESSFULLY PARSED Andrea Paola Ramirez JSON!")
...
with urllib.request.urlopen(prods_url) as resp:
    ...
    for p in res.get('data', []):
        ...
        if 'andrea' in name and 'ramirez' in name:
...
if not andrea_id:
    with urllib.request.urlopen(prods_url) as resp:
        ...
        for p in res.get('data', []):
            name = p.get('nombre_completo', '').lower()
            if 'andrea' in name:
                print(f"Found Andrea Candidate: ID={p['id']}, Name='{p['nombre_completo']}'")
                andrea_id = p['id']

if not andrea_id:
    raise Exception("Andrea Paola Ramirez Parra not found in database!")

print(f"\nTargeting Producer ID: {andrea_id}")

payload = {
    'productor_id': andrea_id,
    'data': data
}
...
with urllib.request.urlopen(req) as resp:
    ...
    print(f"Submit API Response for ID {andrea_id}:", res_text)

# Verify get_pmapc for ID
get_url = f'http://localhost:8000/api/get_pmapc.php?id={andrea_id}'
with urllib.request.urlopen(get_url) as g_resp:
    res = json.loads(g_resp.read().decode('utf-8'))
    print(f"get_pmapc response for ID {andrea_id}: success={res.get('success')}, exists={res.get('exists')}")

# Test PDF Download
pdf_url = f'http://localhost:8000/api/download_pmapc_pdf.php?id={andrea_id}'
with urllib.request.urlopen(pdf_url) as pdf_resp:
    html = pdf_resp.read().decode('utf-8')

print(f"PDF HTML Length for ID {andrea_id}:", len(html), "bytes")

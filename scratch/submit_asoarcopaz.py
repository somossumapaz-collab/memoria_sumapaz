...
path = r'C:\Users\sotoc\Downloads\JSON Asoarcopaz.json'
if not os.path.exists(path):
    files = [f for f in os.listdir(r'C:\Users\sotoc\Downloads') if 'Asoarcopaz' in f or 'asoarco' in f.lower()]
    if files:
...
try:
    ...
    print("SUCCESSFULLY PARSED Asoarcopaz JSON!")
...
with urllib.request.urlopen(prods_url) as resp:
    ...
    for p in res.get('data', []):
        ...
        if 'asoarcopaz' in name or 'asoarcopaz' in org or 'asoarco' in name or 'asoarco' in org:
...
if not aso_id:
    ...
    with urllib.request.urlopen(prods_url) as resp:
        ...
        for p in res.get('data', []):
            name = p.get('nombre_completo', '').lower()
            org = (p.get('nombre_organizacion') or '').lower()
            if 'asociaci' in name or 'asociaci' in org or 'paz' in name or 'paz' in org:
                print(f"Candidate: ID={p['id']}, Name='{p['nombre_completo']}', Org='{p.get('nombre_organizacion')}'")

if not aso_id:
    raise Exception("Asociación Asoarcopaz not found in database!")

...
$json_path = 'C:\Users\sotoc\Downloads\JSON Beatriz Romero.json';
...
echo "=== INSPECTING BEATRIZ ROMERO LÓPEZ JSON ===\n";
...
$stmt = $pdo->query("SELECT id, nombre_completo, vereda, numero_documento FROM productores_sumapaz WHERE nombre_completo LIKE '%Beatriz%' OR nombre_completo LIKE '%Romero%' OR nombre_completo LIKE '%López%' OR nombre_completo LIKE '%Lopez%'");
...
foreach ($producers as $p) {
    ...
    if (strpos($name, 'beatriz') !== false && strpos($name, 'romero') !== false) {
        $beatriz_id = $p['id'];
        break;
    }
}

if (!$beatriz_id && !empty($producers)) {
    foreach ($producers as $p) {
        if (strpos(strtolower($p['nombre_completo']), 'beatriz') !== false) {
            $beatriz_id = $p['id'];
            break;
        }
    }
...
echo "Targeting Producer ID: " . ($beatriz_id ? $beatriz_id : "NOT FOUND") . "\n";

if (!$beatriz_id) {
    echo "Creating producer record for Beatriz Romero López...\n";
    $stmtInsProd = $pdo->prepare("INSERT INTO productores_sumapaz (nombre_completo, vereda, tipo_productor) VALUES (?, ?, ?)");
    $stmtInsProd->execute(['Beatriz Romero López', 'Santo Domingo', 'Individual']);
    $beatriz_id = $pdo->lastInsertId();
    echo "Created producer with ID: {$beatriz_id}\n";
}
...
$nombreOrg = $f01['nombre_unidad_productiva'] ?? ($f01['nombre_organizacion'] ?? 'Santanita');
...
");
$stmtMaster->execute([$beatriz_id, $nombreOrg, $estadoAct, $jsonData]);

// Fetch registro_id
$stmtRegId = $pdo->prepare("SELECT id FROM pmapc_registros WHERE productor_id = ?");
$stmtRegId->execute([$beatriz_id]);
$registro_id = $stmtRegId->fetchColumn();

echo "Saved to pmapc_registros table successfully (registro_id = {$registro_id}, productor_id = {$beatriz_id})!\n";

// Populate relational tables via API submit script logic
function nan_val_br($val) {
...
");
$stmtEst->execute([
    $registro_id, $beatriz_id,
    nan_val_br($f01['nombre_unidad_productiva'] ?? ''),
    nan_val_br($f01['tipo_actividad'] ?? ''),
    nan_val_br($f01['ubicacion_especifica'] ?? ''),
    nan_val_br($f01['coordenadas'] ?? ''),
    nan_val_br($f01['producto_servicio_principal'] ?? ''),
    nan_val_br($f01['estado_actual'] ?? ''),
    nan_val_br($f01['descripcion_general'] ?? ''),
    nan_val_br($f02['mision'] ?? ''),
    nan_val_br($f02['vision'] ?? ''),
    nan_val_br($f02['valores'] ?? ''),
    nan_val_br($f03['por_que_adquieren_el_servicio'] ?? ''),
    nan_val_br($f03['beneficio_cliente'] ?? ''),
    nan_val_br($f03['diferencial'] ?? ''),
    nan_val_br($f03['valor_ambiental'] ?? ''),
    nan_val_br($f03['valor_social_comunitario'] ?? ''),
    nan_val_br($f03['evidencia'] ?? ''),
    nan_val_br($f04['fortalezas'] ?? ''),
    nan_val_br($f04['oportunidades'] ?? ''),
    nan_val_br($f04['debilidades'] ?? ''),
    nan_val_br($f04['amenazas'] ?? '')
]);
...
$stmtCom->execute([
    $registro_id,
    $beatriz_id,
    'C:\Users\sotoc\Downloads\JSON Beatriz Romero.json',
    nan_val_br($f01['observaciones_o_comentarios'] ?? $f03['observaciones_o_comentarios'] ?? ''),
    nan_val_br($f03['observaciones_o_comentarios'] ?? ''),
    'NaN',
    'NaN'
]);

echo "SUCCESS! Relational tables populated for Beatriz Romero López (Producer ID: {$beatriz_id}, Registro ID: {$registro_id}).\n";

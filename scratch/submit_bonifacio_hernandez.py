...
path = r'C:\Users\sotoc\Downloads\JSON Bonifacio Hernandez.json'
if not os.path.exists(path):
    files = [f for f in os.listdir(r'C:\Users\sotoc\Downloads') if 'Bonifacio' in f or 'Hernandez' in f]
    if files:
...
try:
    ...
    print("SUCCESSFULLY PARSED Bonifacio Hernandez JSON!")
...
if not bonifacio_id:
    raise Exception("Bonifacio Hernández not found in database!")

print(f"\nTargeting Producer ID: {bonifacio_id}")

payload = {
    'productor_id': bonifacio_id,
    'data': data
}
...
with urllib.request.urlopen(req) as resp:
    ...
    print(f"Submit API Response for ID {bonifacio_id}:", res_text)

# Verify get_pmapc for ID
get_url = f'http://localhost:8000/api/get_pmapc.php?id={bonifacio_id}'
with urllib.request.urlopen(get_url) as g_resp:
    res = json.loads(g_resp.read().decode('utf-8'))
    print(f"get_pmapc response for ID {bonifacio_id}: success={res.get('success')}, exists={res.get('exists')}")

# Test PDF Download
pdf_url = f'http://localhost:8000/api/download_pmapc_pdf.php?id={bonifacio_id}'
with urllib.request.urlopen(pdf_url) as pdf_resp:
    html = pdf_resp.read().decode('utf-8')

print(f"PDF HTML Length for ID {bonifacio_id}:", len(html), "bytes")

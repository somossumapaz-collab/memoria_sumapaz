...
if (json_last_error() !== JSON_ERROR_NONE) {
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        die("Fatal JSON syntax error in Carlos Arturo JSON: " . json_last_error_msg() . "\n");
    } else {
        echo "Fixed JSON syntax successfully!\n";
    }
}

echo "=== INSPECTING CARLOS ARTURO PULIDO TORRES JSON ===\n";
...
// Find Carlos Arturo Pulido Torres in productores_sumapaz
$stmt = $pdo->query("SELECT id, nombre_completo, vereda, numero_documento FROM productores_sumapaz WHERE nombre_completo LIKE '%Carlos%' OR nombre_completo LIKE '%Pulido%'");
...
foreach ($producers as $p) {
    if (strpos(strtolower($p['nombre_completo']), 'carlos') !== false && strpos(strtolower($p['nombre_completo']), 'arturo') !== false) {
        $carlos_id = $p['id'];
        break;
    }
}
...
if (!$carlos_id && !empty($producers)) {
    foreach ($producers as $p) {
        if (strpos(strtolower($p['nombre_completo']), 'carlos') !== false) {
            $carlos_id = $p['id'];
            break;
        }
    }
...
if (!$carlos_id) {
    die("Carlos Arturo Pulido Torres not found in database!\n");
}

echo "\nTargeting Producer ID: {$carlos_id}\n";
...
if ($existingId) {
    ...
    echo "Successfully UPDATED existing PMAPC record ID {$existingId} for producer {$carlos_id}.\n";
} else {
    $stmtIn = $pdo->prepare("INSERT INTO pmapc_registros (productor_id, data) VALUES (?, ?)");
    $stmtIn->execute([$carlos_id, $jsonData]);
    echo "Successfully INSERTED new PMAPC record for producer {$carlos_id}.\n";
}

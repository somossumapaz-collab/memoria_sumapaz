...
$json_path = 'C:\Users\sotoc\Downloads\JSON Claudia Zambrano Torres.json';
...
echo "=== INSPECTING CLAUDIA ZAMBRANO TORRES JSON ===\n";
...
$stmt = $pdo->query("SELECT id, nombre_completo, vereda, numero_documento FROM productores_sumapaz WHERE nombre_completo LIKE '%Claudia%' OR nombre_completo LIKE '%Zambrano%' OR nombre_completo LIKE '%Torres%'");
...
foreach ($producers as $p) {
    ...
    if (strpos($name, 'claudia') !== false && strpos($name, 'zambrano') !== false) {
        $claudia_id = $p['id'];
        break;
    }
}

if (!$claudia_id && !empty($producers)) {
    foreach ($producers as $p) {
        if (strpos(strtolower($p['nombre_completo']), 'claudia') !== false) {
            $claudia_id = $p['id'];
            break;
        }
    }
...
echo "Targeting Producer ID: " . ($claudia_id ? $claudia_id : "NOT FOUND") . "\n";

if (!$claudia_id) {
    echo "Creating producer record for Claudia Zambrano Torres...\n";
    $stmtInsProd = $pdo->prepare("INSERT INTO productores_sumapaz (nombre_completo, vereda, tipo_productor) VALUES (?, ?, ?)");
    $stmtInsProd->execute(['Claudia Zambrano Torres', 'La Unión', 'Individual']);
    $claudia_id = $pdo->lastInsertId();
    echo "Created producer with ID: {$claudia_id}\n";
}
...
$nombreOrg = $f01['nombre_unidad_productiva'] ?? ($f01['nombre_organizacion'] ?? 'Capri Nick Sumapaz');
...
");
$stmtMaster->execute([$claudia_id, $nombreOrg, $estadoAct, $jsonData]);

// Fetch registro_id
$stmtRegId = $pdo->prepare("SELECT id FROM pmapc_registros WHERE productor_id = ?");
$stmtRegId->execute([$claudia_id]);
$registro_id = $stmtRegId->fetchColumn();

echo "Saved to pmapc_registros table successfully (registro_id = {$registro_id}, productor_id = {$claudia_id})!\n";

// Populate relational tables via API submit script logic
function nan_val_cz($val) {
...
");
$stmtEst->execute([
    $registro_id, $claudia_id,
    nan_val_cz($f01['nombre_unidad_productiva'] ?? ''),
    nan_val_cz($f01['tipo_actividad'] ?? ''),
    nan_val_cz($f01['ubicacion_especifica'] ?? ''),
    nan_val_cz($f01['coordenadas'] ?? ''),
    nan_val_cz($f01['producto_servicio_principal'] ?? ''),
    nan_val_cz($f01['estado_actual'] ?? ''),
    nan_val_cz($f01['descripcion_general'] ?? ''),
    nan_val_cz($f02['mision'] ?? ''),
    nan_val_cz($f02['vision'] ?? ''),
    nan_val_cz($f02['valores'] ?? ''),
    nan_val_cz($f03['por_que_adquieren_el_servicio'] ?? ''),
    nan_val_cz($f03['beneficio_cliente'] ?? ''),
    nan_val_cz($f03['diferencial'] ?? ''),
    nan_val_cz($f03['valor_ambiental'] ?? ''),
    nan_val_cz($f03['valor_social_comunitario'] ?? ''),
    nan_val_cz($f03['evidencia'] ?? ''),
    nan_val_cz($f04['fortalezas'] ?? ''),
    nan_val_cz($f04['oportunidades'] ?? ''),
    nan_val_cz($f04['debilidades'] ?? ''),
    nan_val_cz($f04['amenazas'] ?? '')
]);
...
$stmtCom->execute([
    $registro_id,
    $claudia_id,
    'C:\Users\sotoc\Downloads\JSON Claudia Zambrano Torres.json',
    nan_val_cz($f01['observaciones_o_comentarios'] ?? $f03['observaciones_o_comentarios'] ?? ''),
    nan_val_cz($f03['observaciones_o_comentarios'] ?? ''),
    'NaN',
    'NaN'
]);

echo "SUCCESS! Relational tables populated for Claudia Zambrano Torres (Producer ID: {$claudia_id}, Registro ID: {$registro_id}).\n";

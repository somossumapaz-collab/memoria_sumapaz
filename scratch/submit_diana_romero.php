...
$json_path = 'C:\Users\sotoc\Downloads\JSON Diana Marcela Romero.json';
...
echo "=== INSPECTING DIANA MARCELA ROMERO JSON ===\n";
...
$stmt = $pdo->query("SELECT id, nombre_completo, vereda, numero_documento FROM productores_sumapaz WHERE nombre_completo LIKE '%Diana%' OR nombre_completo LIKE '%Marcela%' OR nombre_completo LIKE '%Romero%'");
...
foreach ($producers as $p) {
    ...
    if (strpos($name, 'diana') !== false && strpos($name, 'romero') !== false) {
        $diana_id = $p['id'];
        break;
    }
}

if (!$diana_id && !empty($producers)) {
    foreach ($producers as $p) {
        if (strpos(strtolower($p['nombre_completo']), 'diana') !== false) {
            $diana_id = $p['id'];
            break;
        }
    }
...
echo "Targeting Producer ID: " . ($diana_id ? $diana_id : "NOT FOUND") . "\n";

if (!$diana_id) {
    echo "Creating producer record for Diana Marcela Romero...\n";
    $stmtInsProd = $pdo->prepare("INSERT INTO productores_sumapaz (nombre_completo, vereda, tipo_productor) VALUES (?, ?, ?)");
    $stmtInsProd->execute(['Diana Marcela Romero', 'Peñaliza', 'Individual']);
    $diana_id = $pdo->lastInsertId();
    echo "Created producer with ID: {$diana_id}\n";
}
...
$nombreOrg = $f01['nombre_unidad_productiva'] ?? ($f01['nombre_organizacion'] ?? 'Panadería La Esquina');
...
");
$stmtMaster->execute([$diana_id, $nombreOrg, $estadoAct, $jsonData]);

// Fetch registro_id
$stmtRegId = $pdo->prepare("SELECT id FROM pmapc_registros WHERE productor_id = ?");
$stmtRegId->execute([$diana_id]);
$registro_id = $stmtRegId->fetchColumn();

echo "Saved to pmapc_registros table successfully (registro_id = {$registro_id}, productor_id = {$diana_id})!\n";

// Populate relational tables via API submit script logic
function nan_val_dmr($val) {
...
");
$stmtEst->execute([
    $registro_id, $diana_id,
    nan_val_dmr($f01['nombre_unidad_productiva'] ?? ''),
    nan_val_dmr($f01['tipo_actividad'] ?? ''),
    nan_val_dmr($f01['ubicacion_especifica'] ?? ''),
    nan_val_dmr($f01['coordenadas'] ?? ''),
    nan_val_dmr($f01['producto_servicio_principal'] ?? ''),
    nan_val_dmr($f01['estado_actual'] ?? ''),
    nan_val_dmr($f01['descripcion_general'] ?? ''),
    nan_val_dmr($f02['mision'] ?? ''),
    nan_val_dmr($f02['vision'] ?? ''),
    nan_val_dmr($f02['valores'] ?? ''),
    nan_val_dmr($f03['por_que_adquieren_el_servicio'] ?? ''),
    nan_val_dmr($f03['beneficio_cliente'] ?? ''),
    nan_val_dmr($f03['diferencial'] ?? ''),
    nan_val_dmr($f03['valor_ambiental'] ?? ''),
    nan_val_dmr($f03['valor_social_comunitario'] ?? ''),
    nan_val_dmr($f03['evidencia'] ?? ''),
    nan_val_dmr($f04['fortalezas'] ?? ''),
    nan_val_dmr($f04['oportunidades'] ?? ''),
    nan_val_dmr($f04['debilidades'] ?? ''),
    nan_val_dmr($f04['amenazas'] ?? '')
]);
...
$stmtCom->execute([
    $registro_id,
    $diana_id,
    'C:\Users\sotoc\Downloads\JSON Diana Marcela Romero.json',
    nan_val_dmr($f01['observaciones_o_comentarios'] ?? $f03['observaciones_o_comentarios'] ?? ''),
    nan_val_dmr($f03['observaciones_o_comentarios'] ?? ''),
    'NaN',
    'NaN'
]);

echo "SUCCESS! Relational tables populated for Diana Marcela Romero (Producer ID: {$diana_id}, Registro ID: {$registro_id}).\n";

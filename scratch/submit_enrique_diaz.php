...
$json_path = 'C:\Users\sotoc\Downloads\JSON Enrique Diaz.json';
...
echo "=== INSPECTING ENRIQUE DÍAZ JSON ===\n";
...
$stmt = $pdo->query("SELECT id, nombre_completo, vereda, numero_documento FROM productores_sumapaz WHERE nombre_completo LIKE '%Enrique%' OR nombre_completo LIKE '%Díaz%' OR nombre_completo LIKE '%Diaz%'");
...
foreach ($producers as $p) {
    ...
    if (strpos($name, 'enrique') !== false && (strpos($name, 'díaz') !== false || strpos($name, 'diaz') !== false)) {
        $enrique_id = $p['id'];
        break;
    }
}

if (!$enrique_id && !empty($producers)) {
    foreach ($producers as $p) {
        if (strpos(strtolower($p['nombre_completo']), 'enrique') !== false) {
            $enrique_id = $p['id'];
            break;
        }
    }
...
echo "Targeting Producer ID: " . ($enrique_id ? $enrique_id : "NOT FOUND") . "\n";

if (!$enrique_id) {
    echo "Creating producer record for Enrique Díaz...\n";
    $stmtInsProd = $pdo->prepare("INSERT INTO productores_sumapaz (nombre_completo, vereda, tipo_productor) VALUES (?, ?, ?)");
    $stmtInsProd->execute(['Enrique Díaz', 'Ánimas', 'Individual']);
    $enrique_id = $pdo->lastInsertId();
    echo "Created producer with ID: {$enrique_id}\n";
}
...
$nombreOrg = $f01['nombre_unidad_productiva'] ?? ($f01['nombre_organizacion'] ?? 'Cuerpo Sumavida');
...
");
$stmtMaster->execute([$enrique_id, $nombreOrg, $estadoAct, $jsonData]);

// Fetch registro_id
$stmtRegId = $pdo->prepare("SELECT id FROM pmapc_registros WHERE productor_id = ?");
$stmtRegId->execute([$enrique_id]);
$registro_id = $stmtRegId->fetchColumn();

echo "Saved to pmapc_registros table successfully (registro_id = {$registro_id}, productor_id = {$enrique_id})!\n";

// Populate relational tables via API submit script logic
function nan_val_ed($val) {
...
");
$stmtEst->execute([
    $registro_id, $enrique_id,
    nan_val_ed($f01['nombre_unidad_productiva'] ?? ''),
    nan_val_ed($f01['tipo_actividad'] ?? ''),
    nan_val_ed($f01['ubicacion_especifica'] ?? ''),
    nan_val_ed($f01['coordenadas'] ?? ''),
    nan_val_ed($f01['producto_servicio_principal'] ?? ''),
    nan_val_ed($f01['estado_actual'] ?? ''),
    nan_val_ed($f01['descripcion_general'] ?? ''),
    nan_val_ed($f02['mision'] ?? ''),
    nan_val_ed($f02['vision'] ?? ''),
    nan_val_ed($f02['valores'] ?? ''),
    nan_val_ed($f03['por_que_adquieren_el_servicio'] ?? ''),
    nan_val_ed($f03['beneficio_cliente'] ?? ''),
    nan_val_ed($f03['diferencial'] ?? ''),
    nan_val_ed($f03['valor_ambiental'] ?? ''),
    nan_val_ed($f03['valor_social_comunitario'] ?? ''),
    nan_val_ed($f03['evidencia'] ?? ''),
    nan_val_ed($f04['fortalezas'] ?? ''),
    nan_val_ed($f04['oportunidades'] ?? ''),
    nan_val_ed($f04['debilidades'] ?? ''),
    nan_val_ed($f04['amenazas'] ?? '')
]);
...
$stmtCom->execute([
    $registro_id,
    $enrique_id,
    'C:\Users\sotoc\Downloads\JSON Enrique Diaz.json',
    nan_val_ed($f01['observaciones_o_comentarios'] ?? $f03['observaciones_o_comentarios'] ?? ''),
    nan_val_ed($f03['observaciones_o_comentarios'] ?? ''),
    'NaN',
    'NaN'
]);

echo "SUCCESS! Relational tables populated for Enrique Díaz (Producer ID: {$enrique_id}, Registro ID: {$registro_id}).\n";

...
path = r'C:\Users\sotoc\Downloads\JSON Jessica Lorena Salgado.json'
if not os.path.exists(path):
    files = [f for f in os.listdir(r'C:\Users\sotoc\Downloads') if 'Jessica' in f or 'Salgado' in f]
    if files:
...
try:
    ...
    print("SUCCESSFULLY PARSED Jessica Lorena Salgado JSON!")
...
with urllib.request.urlopen(prods_url) as resp:
    ...
    for p in res.get('data', []):
        ...
        if 'jessica' in name and 'salgado' in name:
...
if not jessica_id:
    with urllib.request.urlopen(prods_url) as resp:
        ...
        for p in res.get('data', []):
            name = p.get('nombre_completo', '').lower()
            if 'salgado' in name or 'jessica' in name:
                print(f"Found Salgado Candidate: ID={p['id']}, Name='{p['nombre_completo']}'")
                jessica_id = p['id']

if not jessica_id:
    raise Exception("Jessica Lorena Salgado Contreras not found in database!")

print(f"\nTargeting Producer ID: {jessica_id}")

payload = {
    'productor_id': jessica_id,
    'data': data
}
...
with urllib.request.urlopen(req) as resp:
    ...
    print(f"Submit API Response for ID {jessica_id}:", res_text)

# Verify get_pmapc for ID
get_url = f'http://localhost:8000/api/get_pmapc.php?id={jessica_id}'
with urllib.request.urlopen(get_url) as g_resp:
    res = json.loads(g_resp.read().decode('utf-8'))
    print(f"get_pmapc response for ID {jessica_id}: success={res.get('success')}, exists={res.get('exists')}")

# Test PDF Download
pdf_url = f'http://localhost:8000/api/download_pmapc_pdf.php?id={jessica_id}'
with urllib.request.urlopen(pdf_url) as pdf_resp:
    html = pdf_resp.read().decode('utf-8')

print(f"PDF HTML Length for ID {jessica_id}:", len(html), "bytes")

...
path = r'C:\Users\sotoc\Downloads\JSON Leydy Paola Villalba.json'
if not os.path.exists(path):
    files = [f for f in os.listdir(r'C:\Users\sotoc\Downloads') if 'Leydy' in f or 'Leidy' in f]
    if files:
...
try:
    ...
    print("SUCCESSFULLY PARSED Leydy Paola Villalba JSON!")
...
with urllib.request.urlopen(prods_url) as resp:
    ...
    for p in res.get('data', []):
        ...
        if ('leidy' in name or 'leydy' in name) and 'villalba' in name:
...
if not leidy_id:
    with urllib.request.urlopen(prods_url) as resp:
        ...
        for p in res.get('data', []):
            name = p.get('nombre_completo', '').lower()
            if 'villalba' in name:
                print(f"Found Villalba Candidate: ID={p['id']}, Name='{p['nombre_completo']}'")
                leidy_id = p['id']

if not leidy_id:
    raise Exception("Leidy Paola Villalba Sierra not found in database!")

print(f"\nTargeting Producer ID: {leidy_id}")

payload = {
    'productor_id': leidy_id,
    'data': data
}
...
with urllib.request.urlopen(req) as resp:
    ...
    print(f"Submit API Response for ID {leidy_id}:", res_text)

# Verify get_pmapc for ID
get_url = f'http://localhost:8000/api/get_pmapc.php?id={leidy_id}'
with urllib.request.urlopen(get_url) as g_resp:
    res = json.loads(g_resp.read().decode('utf-8'))
    print(f"get_pmapc response for ID {leidy_id}: success={res.get('success')}, exists={res.get('exists')}")

# Test PDF Download
pdf_url = f'http://localhost:8000/api/download_pmapc_pdf.php?id={leidy_id}'
with urllib.request.urlopen(pdf_url) as pdf_resp:
    html = pdf_resp.read().decode('utf-8')

print(f"PDF HTML Length for ID {leidy_id}:", len(html), "bytes")

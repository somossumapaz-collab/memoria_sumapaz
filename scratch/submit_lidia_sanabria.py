...
path = r'C:\Users\sotoc\Downloads\JSON Lidia Zanabria.json'
if not os.path.exists(path):
    files = [f for f in os.listdir(r'C:\Users\sotoc\Downloads') if 'Lidia' in f or 'Zanabria' in f or 'Sanabria' in f]
    if files:
...
try:
    ...
    print("SUCCESSFULLY PARSED Lidia Zanabria JSON!")
...
with urllib.request.urlopen(prods_url) as resp:
    ...
    for p in res.get('data', []):
        ...
        if 'lidia' in name and ('sanabria' in name or 'zanabria' in name):
...
if not lidia_id:
    with urllib.request.urlopen(prods_url) as resp:
        ...
        for p in res.get('data', []):
            name = p.get('nombre_completo', '').lower()
            if 'lidia' in name:
                print(f"Found Lidia Candidate: ID={p['id']}, Name='{p['nombre_completo']}'")
                lidia_id = p['id']

if not lidia_id:
    raise Exception("Lidia Sanabria Lopez not found in database!")

print(f"\nTargeting Producer ID: {lidia_id}")

payload = {
    'productor_id': lidia_id,
    'data': data
}
...
with urllib.request.urlopen(req) as resp:
    ...
    print(f"Submit API Response for ID {lidia_id}:", res_text)

# Verify get_pmapc for ID
get_url = f'http://localhost:8000/api/get_pmapc.php?id={lidia_id}'
with urllib.request.urlopen(get_url) as g_resp:
    res = json.loads(g_resp.read().decode('utf-8'))
    print(f"get_pmapc response for ID {lidia_id}: success={res.get('success')}, exists={res.get('exists')}")

# Test PDF Download
pdf_url = f'http://localhost:8000/api/download_pmapc_pdf.php?id={lidia_id}'
with urllib.request.urlopen(pdf_url) as pdf_resp:
    html = pdf_resp.read().decode('utf-8')

print(f"PDF HTML Length for ID {lidia_id}:", len(html), "bytes")

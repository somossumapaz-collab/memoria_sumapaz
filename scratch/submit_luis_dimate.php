...
$json_path = 'C:\Users\sotoc\Downloads\JSON Luis Eduardo Dimate.json';
...
echo "=== INSPECTING LUIS EDUARDO DIMATÉ JSON ===\n";
...
$stmt = $pdo->query("SELECT id, nombre_completo, vereda, numero_documento FROM productores_sumapaz WHERE nombre_completo LIKE '%Luis%' OR nombre_completo LIKE '%Eduardo%' OR nombre_completo LIKE '%Dimaté%' OR nombre_completo LIKE '%Dimate%'");
...
foreach ($producers as $p) {
    ...
    if (strpos($name, 'luis') !== false && strpos($name, 'eduardo') !== false && (strpos($name, 'dimate') !== false || strpos($name, 'dimaté') !== false)) {
        $luis_id = $p['id'];
        break;
    }
}
...
if (!$luis_id && !empty($producers)) {
    foreach ($producers as $p) {
        if (strpos(strtolower($p['nombre_completo']), 'luis eduardo dimate') !== false || (strpos(strtolower($p['nombre_completo']), 'luis') !== false && strpos(strtolower($p['nombre_completo']), 'dimate') !== false)) {
            $luis_id = $p['id'];
            break;
        }
    }
...
if (!$luis_id) {
    echo "Creating producer record for Luis Eduardo Dimaté...\n";
    $stmtInsProd = $pdo->prepare("INSERT INTO productores_sumapaz (nombre_completo, vereda, tipo_productor) VALUES (?, ?, ?)");
    $stmtInsProd->execute(['Luis Eduardo Dimaté', 'Las Ánimas', 'Individual']);
...
$nombreOrg = $f01['nombre_unidad_productiva'] ?? ($f01['nombre_organizacion'] ?? 'Unidad productiva de Luis Eduardo Dimaté');
...
$stmtEst->execute([
    $registro_id, $luis_id,
    nan_val_led($f01['nombre_unidad_productiva'] ?? ''),
    nan_val_led($f01['tipo_actividad'] ?? ''),
    nan_val_led($f01['ubicacion_especifica'] ?? ''),
    nan_val_led($f01['coordenadas'] ?? ''),
    nan_val_led($f01['producto_servicio_principal'] ?? ''),
    nan_val_led($f01['estado_actual'] ?? ''),
    nan_val_led($f01['descripcion_general'] ?? ''),
    nan_val_led($f02['mision'] ?? ''),
    nan_val_led($f02['vision'] ?? ''),
    nan_val_led($f02['valores'] ?? ''),
    nan_val_led($f03['por_que_adquieren_el_servicio'] ?? ''),
    nan_val_led($f03['beneficio_cliente'] ?? ''),
    nan_val_led($f03['diferencial'] ?? ''),
    nan_val_led($f03['valor_ambiental'] ?? ''),
    nan_val_led($f03['valor_social_comunitario'] ?? ''),
    nan_val_led($f03['evidencia'] ?? ''),
    nan_val_led($f04['fortalezas'] ?? ''),
    nan_val_led($f04['oportunidades'] ?? ''),
    nan_val_led($f04['debilidades'] ?? ''),
    nan_val_led($f04['amenazas'] ?? '')
]);
...
$stmtCom->execute([
    $registro_id,
    $luis_id,
    'C:\Users\sotoc\Downloads\JSON Luis Eduardo Dimate.json',
    nan_val_led($f01['observaciones_o_comentarios'] ?? $f03['observaciones_o_comentarios'] ?? ''),
    nan_val_led($f03['observaciones_o_comentarios'] ?? ''),
    'NaN',
    'NaN'
]);

echo "SUCCESS! Relational tables populated for Luis Eduardo Dimaté (Producer ID: {$luis_id}, Registro ID: {$registro_id}).\n";

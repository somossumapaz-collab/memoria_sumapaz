...
path = r'C:\Users\sotoc\Downloads\JSON Maria Luz Dary Zambrano.json'
if not os.path.exists(path):
    files = [f for f in os.listdir(r'C:\Users\sotoc\Downloads') if 'Zambrano' in f or 'Luz Dary' in f]
    if files:
...
try:
    ...
    print("SUCCESSFULLY PARSED Maria Luz Dary Zambrano JSON!")
...
with urllib.request.urlopen(prods_url) as resp:
    ...
    for p in res.get('data', []):
        ...
        if ('luz dary' in name or 'zambrano' in name) and ('maria' in name or 'maría' in name):
...
if not marialuz_id:
    with urllib.request.urlopen(prods_url) as resp:
        ...
        for p in res.get('data', []):
            name = p.get('nombre_completo', '').lower()
            if 'zambrano' in name:
                print(f"Found Zambrano Candidate: ID={p['id']}, Name='{p['nombre_completo']}'")
                marialuz_id = p['id']

if not marialuz_id:
    raise Exception("Maria Luz Dary Zambrano Torres not found in database!")

print(f"\nTargeting Producer ID: {marialuz_id}")

payload = {
    'productor_id': marialuz_id,
    'data': data
}
...
with urllib.request.urlopen(req) as resp:
    ...
    print(f"Submit API Response for ID {marialuz_id}:", res_text)

# Verify get_pmapc for ID
get_url = f'http://localhost:8000/api/get_pmapc.php?id={marialuz_id}'
with urllib.request.urlopen(get_url) as g_resp:
    res = json.loads(g_resp.read().decode('utf-8'))
    print(f"get_pmapc response for ID {marialuz_id}: success={res.get('success')}, exists={res.get('exists')}")

# Test PDF Download
pdf_url = f'http://localhost:8000/api/download_pmapc_pdf.php?id={marialuz_id}'
with urllib.request.urlopen(pdf_url) as pdf_resp:
    html = pdf_resp.read().decode('utf-8')

print(f"PDF HTML Length for ID {marialuz_id}:", len(html), "bytes")

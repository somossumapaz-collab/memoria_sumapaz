...
$json_path = 'C:\Users\sotoc\Downloads\JSON Mauricio Pabon.json';
...
echo "=== INSPECTING MAURICIO PABÓN MORALES JSON ===\n";
...
// Explicitly targeting Producer ID 268 for Mauricio Pabón Morales (CC: 79519580, Vereda: Betania)
$mauricio_id = 268;

echo "Targeting Producer ID: {$mauricio_id}\n";

// Master table insert/update
$jsonData = json_encode($data, JSON_UNESCAPED_UNICODE);

$f01 = $data['PMAPC_F01'] ?? ($data['f01'] ?? []);
$nombreOrg = $f01['nombre_unidad_productiva'] ?? ($f01['nombre_organizacion'] ?? 'Unidad productiva familiar de fresa Finca El Pedregal');
...
");
$stmtMaster->execute([$mauricio_id, $nombreOrg, $estadoAct, $jsonData]);

// Fetch registro_id
$stmtRegId = $pdo->prepare("SELECT id FROM pmapc_registros WHERE productor_id = ?");
$stmtRegId->execute([$mauricio_id]);
$registro_id = $stmtRegId->fetchColumn();

echo "Saved to pmapc_registros table successfully (registro_id = {$registro_id}, productor_id = {$mauricio_id})!\n";

// Populate relational tables via API submit script logic
function nan_val_mpm($val) {
...
");
$stmtEst->execute([
    $registro_id, $mauricio_id,
    nan_val_mpm($f01['nombre_unidad_productiva'] ?? ''),
    nan_val_mpm($f01['tipo_actividad'] ?? ''),
    nan_val_mpm($f01['ubicacion_especifica'] ?? ''),
    nan_val_mpm($f01['coordenadas'] ?? ''),
    nan_val_mpm($f01['producto_servicio_principal'] ?? ''),
    nan_val_mpm($f01['estado_actual'] ?? ''),
    nan_val_mpm($f01['descripcion_general'] ?? ''),
    nan_val_mpm($f02['mision'] ?? ''),
    nan_val_mpm($f02['vision'] ?? ''),
    nan_val_mpm($f02['valores'] ?? ''),
    nan_val_mpm($f03['por_que_adquieren_el_servicio'] ?? ''),
    nan_val_mpm($f03['beneficio_cliente'] ?? ''),
    nan_val_mpm($f03['diferencial'] ?? ''),
    nan_val_mpm($f03['valor_ambiental'] ?? ''),
    nan_val_mpm($f03['valor_social_comunitario'] ?? ''),
    nan_val_mpm($f03['evidencia'] ?? ''),
    nan_val_mpm($f04['fortalezas'] ?? ''),
    nan_val_mpm($f04['oportunidades'] ?? ''),
    nan_val_mpm($f04['debilidades'] ?? ''),
    nan_val_mpm($f04['amenazas'] ?? '')
]);
...
$stmtCom->execute([
    $registro_id,
    $mauricio_id,
    'C:\Users\sotoc\Downloads\JSON Mauricio Pabon.json',
    nan_val_mpm($f01['observaciones_o_comentarios'] ?? $f03['observaciones_o_comentarios'] ?? ''),
    nan_val_mpm($f03['observaciones_o_comentarios'] ?? ''),
    'NaN',
    'NaN'
]);

echo "SUCCESS! Relational tables populated for Mauricio Pabón Morales (Producer ID: {$mauricio_id}, Registro ID: {$registro_id}).\n";

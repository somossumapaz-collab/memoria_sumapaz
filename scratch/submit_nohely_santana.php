...
$json_path = 'C:\Users\sotoc\Downloads\JSON Nohely Santana.json';
...
echo "=== INSPECTING NOHELY SANTANA SÁNCHEZ JSON ===\n";
...
// Explicitly targeting Producer ID 12 for Nohely Santana Sánchez (CC: 348932, Vereda: La Unión)
$nohely_id = 12;

echo "Targeting Producer ID: {$nohely_id}\n";

// Master table insert/update
$jsonData = json_encode($data, JSON_UNESCAPED_UNICODE);

$f01 = $data['PMAPC_F01'] ?? ($data['f01'] ?? []);
$nombreOrg = $f01['nombre_unidad_productiva'] ?? ($f01['nombre_organizacion'] ?? 'Manantial de Amor');
...
");
$stmtMaster->execute([$nohely_id, $nombreOrg, $estadoAct, $jsonData]);

// Fetch registro_id
$stmtRegId = $pdo->prepare("SELECT id FROM pmapc_registros WHERE productor_id = ?");
$stmtRegId->execute([$nohely_id]);
$registro_id = $stmtRegId->fetchColumn();

echo "Saved to pmapc_registros table successfully (registro_id = {$registro_id}, productor_id = {$nohely_id})!\n";

// Populate relational tables via API submit script logic
function nan_val_nss($val) {
...
");
$stmtEst->execute([
    $registro_id, $nohely_id,
    nan_val_nss($f01['nombre_unidad_productiva'] ?? ''),
    nan_val_nss($f01['tipo_actividad'] ?? ''),
    nan_val_nss($f01['ubicacion_especifica'] ?? ''),
    nan_val_nss($f01['coordenadas'] ?? ''),
    nan_val_nss($f01['producto_servicio_principal'] ?? ''),
    nan_val_nss($f01['estado_actual'] ?? ''),
    nan_val_nss($f01['descripcion_general'] ?? ''),
    nan_val_nss($f02['mision'] ?? ''),
    nan_val_nss($f02['vision'] ?? ''),
    nan_val_nss($f02['valores'] ?? ''),
    nan_val_nss($f03['por_que_adquieren_el_servicio'] ?? ''),
    nan_val_nss($f03['beneficio_cliente'] ?? ''),
    nan_val_nss($f03['diferencial'] ?? ''),
    nan_val_nss($f03['valor_ambiental'] ?? ''),
    nan_val_nss($f03['valor_social_comunitario'] ?? ''),
    nan_val_nss($f03['evidencia'] ?? ''),
    nan_val_nss($f04['fortalezas'] ?? ''),
    nan_val_nss($f04['oportunidades'] ?? ''),
    nan_val_nss($f04['debilidades'] ?? ''),
    nan_val_nss($f04['amenazas'] ?? '')
]);
...
$stmtCom->execute([
    $registro_id,
    $nohely_id,
    'C:\Users\sotoc\Downloads\JSON Nohely Santana.json',
    nan_val_nss($f01['observaciones_o_comentarios'] ?? $f03['observaciones_o_comentarios'] ?? ''),
    nan_val_nss($f03['observaciones_o_comentarios'] ?? ''),
    'NaN',
    'NaN'
]);

echo "SUCCESS! Relational tables populated for Nohely Santana Sánchez (Producer ID: {$nohely_id}, Registro ID: {$registro_id}).\n";

...
path = r'C:\Users\sotoc\Downloads\JSON Paula Daniela Larrota.json'
...
try:
    ...
    print("SUCCESSFULLY PARSED Paula Daniela Larrota JSON!")
...
with urllib.request.urlopen(prods_url) as resp:
    ...
    for p in res.get('data', []):
        ...
        if 'paula' in name and 'larrota' in name:
...
if not paula_id:
    with urllib.request.urlopen(prods_url) as resp:
        ...
        for p in res.get('data', []):
            name = p.get('nombre_completo', '').lower()
            if 'larrota' in name:
                print(f"Found Larrota Candidate: ID={p['id']}, Name='{p['nombre_completo']}'")
                paula_id = p['id']

if not paula_id:
    raise Exception("Paula Daniela Larrota Borray not found in database!")

print(f"\nTargeting Producer ID: {paula_id}")

payload = {
    'productor_id': paula_id,
    'data': data
}
...
with urllib.request.urlopen(req) as resp:
    ...
    print(f"Submit API Response for ID {paula_id}:", res_text)

# Verify get_pmapc for ID
get_url = f'http://localhost:8000/api/get_pmapc.php?id={paula_id}'
with urllib.request.urlopen(get_url) as g_resp:
    res = json.loads(g_resp.read().decode('utf-8'))
    print(f"get_pmapc response for ID {paula_id}: success={res.get('success')}, exists={res.get('exists')}")

# Test PDF Download
pdf_url = f'http://localhost:8000/api/download_pmapc_pdf.php?id={paula_id}'
with urllib.request.urlopen(pdf_url) as pdf_resp:
    html = pdf_resp.read().decode('utf-8')

print(f"PDF HTML Length for ID {paula_id}:", len(html), "bytes")

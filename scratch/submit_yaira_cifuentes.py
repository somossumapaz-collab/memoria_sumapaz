...
path = r'C:\Users\sotoc\Downloads\JSON Yadira Gissele Cifuentes.json'
if not os.path.exists(path):
    files = [f for f in os.listdir(r'C:\Users\sotoc\Downloads') if 'Yadira' in f or 'Yaira' in f or 'Gissele' in f]
    if files:
...
try:
    ...
    print("SUCCESSFULLY PARSED Yadira Gissele Cifuentes JSON!")
...
with urllib.request.urlopen(prods_url) as resp:
    ...
    for p in res.get('data', []):
        ...
        if ('yaira' in name or 'yadira' in name) and 'cifuentes' in name:
...
if not yaira_id:
    with urllib.request.urlopen(prods_url) as resp:
        ...
        for p in res.get('data', []):
            name = p.get('nombre_completo', '').lower()
            if 'cifuentes' in name:
                print(f"Found Cifuentes Candidate: ID={p['id']}, Name='{p['nombre_completo']}'")
                yaira_id = p['id']

if not yaira_id:
    raise Exception("Yaira Yisela Cifuentes Vázquez not found in database!")

print(f"\nTargeting Producer ID: {yaira_id}")

payload = {
    'productor_id': yaira_id,
    'data': data
}
...
with urllib.request.urlopen(req) as resp:
    ...
    print(f"Submit API Response for ID {yaira_id}:", res_text)

# Verify get_pmapc for ID
get_url = f'http://localhost:8000/api/get_pmapc.php?id={yaira_id}'
with urllib.request.urlopen(get_url) as g_resp:
    res = json.loads(g_resp.read().decode('utf-8'))
    print(f"get_pmapc response for ID {yaira_id}: success={res.get('success')}, exists={res.get('exists')}")

# Test PDF Download
pdf_url = f'http://localhost:8000/api/download_pmapc_pdf.php?id={yaira_id}'
with urllib.request.urlopen(pdf_url) as pdf_resp:
    html = pdf_resp.read().decode('utf-8')

print(f"PDF HTML Length for ID {yaira_id}:", len(html), "bytes")

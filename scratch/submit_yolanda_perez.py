...
path = r'C:\Users\sotoc\Downloads\JSON Maria Yolanda Perez.json'
...
try:
    ...
    print("SUCCESSFULLY PARSED Maria Yolanda Perez JSON!")
...
with urllib.request.urlopen(prods_url) as resp:
    ...
    for p in res.get('data', []):
        ...
        if 'yolanda' in name and 'perez' in name:
...
if not yolanda_id:
    with urllib.request.urlopen(prods_url) as resp:
        ...
        for p in res.get('data', []):
            name = p.get('nombre_completo', '').lower()
            if 'yolanda' in name:
                print(f"Found Yolanda Candidate: ID={p['id']}, Name='{p['nombre_completo']}'")
                yolanda_id = p['id']

if not yolanda_id:
    raise Exception("Maria Yolanda Perez Ussa not found in database!")

print(f"\nTargeting Producer ID: {yolanda_id}")

payload = {
    'productor_id': yolanda_id,
    'data': data
}
...
with urllib.request.urlopen(req) as resp:
    ...
    print(f"Submit API Response for ID {yolanda_id}:", res_text)

# Verify get_pmapc for ID
get_url = f'http://localhost:8000/api/get_pmapc.php?id={yolanda_id}'
with urllib.request.urlopen(get_url) as g_resp:
    res = json.loads(g_resp.read().decode('utf-8'))
    print(f"get_pmapc response for ID {yolanda_id}: success={res.get('success')}, exists={res.get('exists')}")

# Test PDF Download
pdf_url = f'http://localhost:8000/api/download_pmapc_pdf.php?id={yolanda_id}'
with urllib.request.urlopen(pdf_url) as pdf_resp:
    html = pdf_resp.read().decode('utf-8')

print(f"PDF HTML Length for ID {yolanda_id}:", len(html), "bytes")

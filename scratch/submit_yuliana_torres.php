...
$json_path = 'C:\Users\sotoc\Downloads\JSON Yuliana Andrea Torres.json';
...
echo "=== INSPECTING YULIANA ANDREA TORRES GUZMÁN JSON ===\n";
...
// Explicitly targeting Producer ID 241 for Yuliana Andrea Torres Guzman (CC: 1003486938, Vereda: La Unión)
$yuliana_id = 241;

echo "Targeting Producer ID: {$yuliana_id}\n";

// Master table insert/update
$jsonData = json_encode($data, JSON_UNESCAPED_UNICODE);

$f01 = $data['PMAPC_F01'] ?? ($data['f01'] ?? []);
$nombreOrg = $f01['nombre_unidad_productiva'] ?? ($f01['nombre_organizacion'] ?? 'Rancho RS');
...
");
$stmtMaster->execute([$yuliana_id, $nombreOrg, $estadoAct, $jsonData]);

// Fetch registro_id
$stmtRegId = $pdo->prepare("SELECT id FROM pmapc_registros WHERE productor_id = ?");
$stmtRegId->execute([$yuliana_id]);
$registro_id = $stmtRegId->fetchColumn();

echo "Saved to pmapc_registros table successfully (registro_id = {$registro_id}, productor_id = {$yuliana_id})!\n";

// Populate relational tables via API submit script logic
function nan_val_yat($val) {
...
");
$stmtEst->execute([
    $registro_id, $yuliana_id,
    nan_val_yat($f01['nombre_unidad_productiva'] ?? ''),
    nan_val_yat($f01['tipo_actividad'] ?? ''),
    nan_val_yat($f01['ubicacion_especifica'] ?? ''),
    nan_val_yat($f01['coordenadas'] ?? ''),
    nan_val_yat($f01['producto_servicio_principal'] ?? ''),
    nan_val_yat($f01['estado_actual'] ?? ''),
    nan_val_yat($f01['descripcion_general'] ?? ''),
    nan_val_yat($f02['mision'] ?? ''),
    nan_val_yat($f02['vision'] ?? ''),
    nan_val_yat($f02['valores'] ?? ''),
    nan_val_yat($f03['por_que_adquieren_el_servicio'] ?? ''),
    nan_val_yat($f03['beneficio_cliente'] ?? ''),
    nan_val_yat($f03['diferencial'] ?? ''),
    nan_val_yat($f03['valor_ambiental'] ?? ''),
    nan_val_yat($f03['valor_social_comunitario'] ?? ''),
    nan_val_yat($f03['evidencia'] ?? ''),
    nan_val_yat($f04['fortalezas'] ?? ''),
    nan_val_yat($f04['oportunidades'] ?? ''),
    nan_val_yat($f04['debilidades'] ?? ''),
    nan_val_yat($f04['amenazas'] ?? '')
]);
...
$stmtCom->execute([
    $registro_id,
    $yuliana_id,
    'C:\Users\sotoc\Downloads\JSON Yuliana Andrea Torres.json',
    nan_val_yat($f01['observaciones_o_comentarios'] ?? $f03['observaciones_o_comentarios'] ?? ''),
    nan_val_yat($f03['observaciones_o_comentarios'] ?? ''),
    'NaN',
    'NaN'
]);

echo "SUCCESS! Relational tables populated for Yuliana Andrea Torres Guzmán (Producer ID: {$yuliana_id}, Registro ID: {$registro_id}).\n";

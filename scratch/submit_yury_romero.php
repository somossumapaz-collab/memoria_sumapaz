...
$json_path = 'C:\Users\sotoc\Downloads\JSON Yuri Romero.json';
...
echo "=== INSPECTING YURY GRACIELA ROMERO BAQUERO JSON ===\n";
...
// Explicitly targeting Producer ID 228 for Yury Graciela Romero Baquero (CC: 1032656461, Vereda: Peñaliza)
$yury_id = 228;

echo "Targeting Producer ID: {$yury_id}\n";

// Master table insert/update
$jsonData = json_encode($data, JSON_UNESCAPED_UNICODE);

$f01 = $data['PMAPC_F01'] ?? ($data['f01'] ?? []);
$nombreOrg = $f01['nombre_unidad_productiva'] ?? ($f01['nombre_organizacion'] ?? 'Ganaderia Familiar Yuri Romero');
...
$stmtEst->execute([
    $registro_id, $yury_id,
    nan_val_yrb($f01['nombre_unidad_productiva'] ?? ''),
    nan_val_yrb($f01['tipo_actividad'] ?? ''),
    nan_val_yrb($f01['ubicacion_especifica'] ?? ''),
    nan_val_yrb($f01['coordenadas'] ?? ''),
    nan_val_yrb($f01['producto_servicio_principal'] ?? ''),
    nan_val_yrb($f01['estado_actual'] ?? ''),
    nan_val_yrb($f01['descripcion_general'] ?? ''),
    nan_val_yrb($f02['mision'] ?? ''),
    nan_val_yrb($f02['vision'] ?? ''),
    nan_val_yrb($f02['valores'] ?? ''),
    nan_val_yrb($f03['por_que_adquieren_el_servicio'] ?? ''),
    nan_val_yrb($f03['beneficio_cliente'] ?? ''),
    nan_val_yrb($f03['diferencial'] ?? ''),
    nan_val_yrb($f03['valor_ambiental'] ?? ''),
    nan_val_yrb($f03['valor_social_comunitario'] ?? ''),
    nan_val_yrb($f03['evidencia'] ?? ''),
    nan_val_yrb($f04['fortalezas'] ?? ''),
    nan_val_yrb($f04['oportunidades'] ?? ''),
    nan_val_yrb($f04['debilidades'] ?? ''),
    nan_val_yrb($f04['amenazas'] ?? '')
]);
...
$stmtCom->execute([
    $registro_id,
    $yury_id,
    'C:\Users\sotoc\Downloads\JSON Yuri Romero.json',
    nan_val_yrb($f01['observaciones_o_comentarios'] ?? $f03['observaciones_o_comentarios'] ?? ''),
    nan_val_yrb($f03['observaciones_o_comentarios'] ?? ''),
    'NaN',
    'NaN'
]);

echo "SUCCESS! Relational tables populated for Yury Graciela Romero Baquero (Producer ID: {$yury_id}, Registro ID: {$registro_id}).\n";

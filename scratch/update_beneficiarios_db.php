...
$stmt = $pdo->query("SELECT id, nombre_completo, numero_documento, vereda, beneficiario_2026 FROM productores_sumapaz");
...
foreach ($raw_list as $input_name) {
    ...
    if ($found_id) {
        ...
        die("ERROR: Name not matched: $input_name\n");
    }
}

$target_ids = array_unique($target_ids);
if (count($target_ids) !== 150) {
    die("ERROR: Expected 150 unique target IDs, got " . count($target_ids) . "\n");
}
...
try {
    // Set beneficiario_2026 = 1 for target IDs
    $in_target = implode(',', $target_ids);
    $stmt1 = $pdo->query("UPDATE productores_sumapaz SET beneficiario_2026 = 1 WHERE id IN ($in_target)");
    $cnt1 = $stmt1->rowCount();
    
    // Set beneficiario_2026 = 0 for non-target IDs
    $stmt0 = $pdo->query("UPDATE productores_sumapaz SET beneficiario_2026 = 0 WHERE id NOT IN ($in_target)");
    $cnt0 = $stmt0->rowCount();

    $pdo->commit();
    echo "SUCCESS: Updated $cnt1 rows to beneficiario_2026 = 1, and $cnt0 rows to beneficiario_2026 = 0.\n";

    // Verify final counts
    $stmt = $pdo->query("SELECT beneficiario_2026, COUNT(*) as cnt FROM productores_sumapaz GROUP BY beneficiario_2026");
    echo "Final database state:\n";
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        echo "  beneficiario_2026 = {$row['beneficiario_2026']} => {$row['cnt']} productores\n";
    }
